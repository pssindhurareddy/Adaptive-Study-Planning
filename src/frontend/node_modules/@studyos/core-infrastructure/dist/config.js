import { ExternalBlob, StorageClient } from "@studyos/object-storage";
import { HttpAgent } from "@icp-sdk/core/agent";
const DEFAULT_STORAGE_GATEWAY_URL = "https://blob.studyos.io";
...
export async function loadConfig() {
    ...
    try {
        ...
        // Automatically fallback to local dfx canister ID so the frontend can function
        if (!backendCanisterId && config.backend_canister_id === "undefined") {
            config.backend_canister_id = "bkyz2-fmaaa-aaaaa-qaaaq-cai";
        }
        const fullConfig = {
        ...
        if (!backendCanisterId) {
            console.warn("CANISTER_ID_BACKEND is not set. Falling back to local canister.");
